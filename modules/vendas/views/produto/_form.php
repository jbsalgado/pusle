...
<div class="produto-form">
    ...
    <div id="tab-content">
        ...
        <?php if (!$model->isNewRecord && $model->fotos): ?>
            <div class="mt-8 pt-8 border-t border-gray-100" id="secao-fotos-existentes">
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
                    <label class="block text-sm font-bold text-gray-800 flex items-center gap-2">
                        ...
                        Fotos Cadastradas <span id="contador-fotos-selecionadas" class="text-xs font-normal text-gray-500">(0 selecionadas)</span>
                    </label>

                    <div class="flex items-center gap-2">
                        <button type="button" id="btn-selecionar-todas-fotos" onclick="toggleSelecionarTodasFotos()" class="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-semibold rounded-lg transition-colors flex items-center gap-1.5 cursor-pointer">
                            <input type="checkbox" id="chk-selecionar-todas-fotos" class="w-3.5 h-3.5 text-blue-600 border-gray-300 rounded focus:ring-blue-500 pointer-events-none">
                            <span>Selecionar Todas</span>
                        </button>

                        <button type="button" id="btn-excluir-fotos-selecionadas" onclick="submeterExclusaoFotosMassa()" disabled class="px-3 py-1.5 bg-red-100 text-red-400 cursor-not-allowed text-xs font-bold rounded-lg transition-all flex items-center gap-1.5 shadow-sm">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                    <?php foreach ($model->fotos as $foto): ?>
                        <div class="relative group aspect-square bg-gray-100 rounded-xl overflow-hidden border border-gray-200 shadow-sm photo-card-item">
                            <!-- Checkbox de seleção na ponta superior esquerda -->
                            <div class="absolute top-2 left-2 z-20 bg-white/90 p-1 rounded-md shadow-md backdrop-blur-sm">
                                <input type="checkbox" name="foto_ids[]" value="<?= $foto->id ?>" class="chk-foto-item w-5 h-5 text-blue-600 bg-white border-2 border-gray-400 rounded focus:ring-blue-500 shadow-sm cursor-pointer" onchange="atualizarContadorFotosSelecionadas()">
                            </div>
                            ...
                            <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-between p-2">
                                <div class="flex justify-end">
                                    <?php $redirectTo = !$model->isNewRecord ? 'update' : 'view'; ?>
