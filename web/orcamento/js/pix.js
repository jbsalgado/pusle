  ...
  try {
    ...
    if (!urlApi) {
      try {
        ...
        const basePath = pathname.replace(/\/orcamento.*$/, "");
    ...
    if (!dados.pix_chave || !dados.pix_nome || !dados.pix_cidade) {
      throw new Error(
        "Configuração PIX não encontrada. Configure os dados PIX nas configurações da loja.",
      );
    }
...
function detectarETratarChavePix(chave) {
  ...
  if (apenasNumeros.length === 10 || apenasNumeros.length === 11) {
    ...
    if (apenasNumeros.length === 10) {
      ...
      if (dddValido && numeroValido) {
        ...
        console.log(
          "[PIX] Chave de TELEFONE FIXO detectada (10 dígitos), formatando para E.164:",
          apenasNumeros,
          "→",
          chaveFormatada,
        );
      ...
      if (dddValido && numeroValido) {
        ...
        const chaveFormatada = "+55" + apenasNumeros;
        console.log(
          "[PIX] Chave de TELEFONE CELULAR detectada (11 dígitos), formatando para E.164:",
          apenasNumeros,
          "→",
          chaveFormatada,
        );
        ...
        console.log(
          "[PIX] Chave de CPF detectada (11 dígitos, não é celular válido):",
          apenasNumeros,
        );
  ...
  console.warn(
    "[PIX] Tipo de chave não identificado, usando original:",
    chaveLimpa,
  );
  return chaveLimpa.replace(/[^0-9a-zA-Z@.+.-]/g, "");
}

export function gerarCodigoPixEstatico(
  chave,
  nome,
  cidade,
  valor = null,
  ...
  const chavePix =
    "01" + String(chaveLimpa.length).padStart(2, "0") + chaveLimpa;
  ...
  const nomeTratado = removerAcentos(nome)
    .substring(0, 25)
    .toUpperCase()
    .trim();
  payload.push(
    "59" + String(nomeTratado.length).padStart(2, "0") + nomeTratado,
  );

  // 8. Merchant City (Obrigatório, ID 60)
  // Remove acentos e limita tamanho (Max 15)
  const cidadeTratada = removerAcentos(cidade)
    .substring(0, 15)
    .toUpperCase()
    .trim();
  payload.push(
    "60" + String(cidadeTratada.length).padStart(2, "0") + cidadeTratada,
  );
  ...
  const txIdField =
    "05" + String(txIdValue.length).padStart(2, "0") + txIdValue;
...
export async function gerarQRCodeVisual(codigoPix, container) {
  try {
    container.innerHTML =
      '<div class="text-center p-4 text-gray-600">Gerando QR Code...</div>';
    ...
    const tryNextApi = () => {
      ...
      if (currentApiIndex < qrCodeApis.length) {
        console.log(
          `[PIX] Tentando API ${currentApiIndex + 1}/${qrCodeApis.length}...`,
        );
    ...
    img.onerror = () => {
      console.warn(
        `[PIX] ⚠️ API ${currentApiIndex + 1} falhou, tentando próxima...`,
      );
      tryNextApi();
    };
 ...
 */
export async function mostrarModalPixEstatico(
  valor,
  txId,
  dadosPedido = null,
  usuarioId = null,
  ...
  if (elContainerQR) {
    elContainerQR.innerHTML =
      '<div class="text-center p-4"><p class="text-gray-600">Carregando configuração PIX...</p></div>';
  }
  if (elCodigo) {
  ...
  try {
    ...
    if (!usuarioId) {
      throw new Error(
        "ID do usuário não encontrado. Não é possível gerar QR Code PIX.",
      );
    }
    ...
    if (elValor)
      elValor.textContent = `R$ ${parseFloat(valor).toFixed(2).replace(".", ",")}`;
    ...
    if (elCodigo) {
      elCodigo.textContent =
        "Erro: Configure os dados PIX nas configurações da loja.";
    }
  }
}
...
window.confirmarRecebimentoPix = async function () {
  if (!dadosPedidoPix) {
    alert(
      "Erro: Dados do pedido não encontrados. Por favor, recarregue a página.",
    );
    return;
  }
  ...
  if (!vendaId) {
    alert(
      "Erro: ID da venda não encontrado. A venda pode não ter sido criada corretamente.",
    );
    return;
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Erro ao confirmar recebimento: ${response.status} - ${errorText}`,
      );
    }
    ...
    if (!parcelas && dadosPedidoPix.numero_parcelas > 1) {
      try {
        ...
        const response = await fetch(
          `${API_ENDPOINTS.PEDIDO_PARCELAS}?venda_id=${vendaId}`,
        );
        if (response.ok) {
    ...
    if (dadosPedidoPix.cliente_id) {
      try {
        const { API_ENDPOINTS } = await import("./config.js");
        const response = await fetch(
          `${API_ENDPOINTS.CLIENTE}/${dadosPedidoPix.cliente_id}`,
        );
        if (response.ok) {
          ...
          dadosCliente = {
            ...
            complemento:
              cliente.endereco_complemento || cliente.complemento || "",
 ...
 */
async function gerarComprovanteOrcamento(carrinho, dadosPedido) {
  console.log(
    "[PIX] 🧾 Gerando comprovante. Parcelas:",
    dadosPedido.parcelas?.length || 0,
  );
  ...
  try {
    ...
    const response = await fetch(
      `${API_ENDPOINTS.USUARIO_DADOS_LOJA}?usuario_id=${CONFIG.ID_USUARIO_LOJA}`,
    );
    if (response.ok) {
    ...
    console.warn(
      "[PIX] Erro ao buscar dados da loja, usando valores padrão:",
      error,
    );
  }
    ...
    if (!logoPath.match(/^https?:\/\//)) {
      ...
      if (window.CONFIG && window.CONFIG.URL_BASE_WEB) {
        ...
        try {
          ...
          console.warn(
            "[PIX] Não foi possível importar CONFIG, usando fallback",
          );
        }
      }
      ...
      if (!baseUrl) {
        ...
        // Remove 'orcamento' ou 'index.html' do final
        pathParts.pop();
        baseUrl =
  ...
  // Formata CPF/CNPJ
  const cpfCnpjLimpo = dadosEmpresa.cpf_cnpj
    ? dadosEmpresa.cpf_cnpj.replace(/[^\d]/g, "")
    : "";
  ...
  const hash = dadosVenda.hash || (dadosPedido && (dadosPedido.hash || (dadosPedido.orçamento && dadosPedido.orçamento.hash)));
  ...
  const acrescimoObs =
    dadosVenda.observacao_acrescimo || dadosPedido.observacao_acrescimo || "";
...
<body>
    ...
    <div class="titulo">COMPROVANTE DE ORÇAMENTO #${idExibicao}</div>
    ...
    <div class="separador">--------------------------------</div>
    <div style="margin: 8px 0;">
        <div style="font-weight: bold; margin-bottom: 3px;">OBSERVAÇÕES:</div>
        <div style="font-size: 11px;">${dadosPedido.observacoes}</div>
    </div>
    ` : ""}
    
    ${(() => {
      const temParcelas =
        dadosPedido.parcelas &&
        Array.isArray(dadosPedido.parcelas) &&
        dadosPedido.parcelas.length > 0;
      ...
      if (!deveMostrar) {
        console.log(
          "[PIX] ℹ️ Venda à vista ou sem parcelas. Tabela de parcelas não será exibida.",
        );
        return "";
      }
                  ...
                  .map((parcela, index) => {
                    try {
                      ...
                      if (isNaN(dataVenc.getTime())) {
                        console.error(
                          "[PIX] ❌ Data inválida para parcela",
                          index + 1,
                          ":",
    ...
    <div class="footer">
        ...
        ${hash ? `
        <div style="margin-top: 10px; padding: 5px; border: 1px dashed #000; font-size: 8px; word-break: break-all;">
            <strong>ACESSO PÚBLICO:</strong><br>
            ${window.location.origin}/vendas/orcamento/imprimir?hash=${hash}
        </div>` : ""}
    </div>
  ...
  iframe.onload = async () => {
    try {
      ...
      if (typeof html2canvas === "undefined") {
        ...
        alert(
          "Erro: Biblioteca html2canvas não carregada. Recarregue a página.",
        );
      ...
      canvas.toBlob(
        (blob) => {
            ...
            container.innerHTML = `
                        <div class="flex flex-col gap-3">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-2">
                                ...
                                ${hash ? `
                                <a href="https://wa.me/?text=${encodeURIComponent('Olá! Segue o link para visualizar seu orçamento: ' + window.location.origin + '/vendas/orcamento/imprimir?hash=' + hash)}" target="_blank" class="bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-4 rounded-lg flex items-center justify-center gap-2 shadow-sm transition-colors">
                                    <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L0 24l6.335-1.662c1.72 1.041 3.512 1.467 5.715 1.467h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                                    WhatsApp (Link)
                                </a>` : `
                                <button disabled class="bg-gray-400 text-white font-bold py-3 px-4 rounded-lg flex items-center justify-center gap-2 shadow-sm cursor-not-allowed">
                                    Compartilhar Link (Indisponível)
                                </button>`}
                            </div>
                        </div>`;
          }
          ...
          if (window.GATEWAY_CONFIG?.imprimir_automatico) {
            console.log(
              "[PIX] 🖨️ Automação: Detectada configuração de impressão automática",
            );
...
function gerarTextoComprovante() {
  ...
  // Cabeçalho
  texto +=
    center(removerAcentos(dadosEmpresa.nome_loja || "LOJA").toUpperCase()) +
    "\n";
  ...
  // Itens
  carrinho.forEach((item) => {
    const nome = removerAcentos(item.nome || "Produto")
      .substring(0, largura)
      .toUpperCase();
    texto += nome + "\n";
    ...
    const preco = parseFloat(
      item.preco_final || item.preco || item.preco_venda_sugerido || 0,
    );
...
/**
 * Gera e abre o orçamento em formato A4 para impressão ou download HTML
 */
window.gerarOrcamentoA4 = function() {
    if (!window.dadosComprovanteAtual) {
        alert("Dados do orçamento não disponíveis.");
        return;
    }

    const { carrinho, dadosPedido, dadosEmpresa, valorTotal, dataHora, acrescimoValor, descontoTotal } = window.dadosComprovanteAtual;
    ...
    const itensHtml = carrinho.map((item, index) => {
        ...
        return `
            <tr class="${index % 2 === 0 ? 'bg-gray-50' : 'bg-white'} border-b">
                <td class="px-4 py-2 text-sm text-gray-700">${item.nome || item.descricao || 'Produto'}</td>
    ...
    // AJEITAR ID: Prioriza ID do servidor (venda_id/id) > ID Local (offline)
    let idOrcamento = dadosPedido.venda_id || dadosPedido.id || dadosPedido.id_local || null;
    let idExibicao = idOrcamento ? (!isNaN(idOrcamento) ? idOrcamento : `OFF-${idOrcamento.substring(0, 8).toUpperCase()}`) : "PENDENTE";
    ...
    <head>
        ...
        <title>Orçamento #${idExibicao}</title>
    ...
    <body class="bg-gray-100 min-h-screen p-8">
        ...
        <div class="max-w-[210mm] mx-auto bg-white shadow-lg p-[10mm] min-h-[297mm] relative">
            ...
            <div class="flex justify-between items-start border-b pb-6 mb-6">
                ...
                <div class="text-right">
                    <h2 class="text-xl font-bold text-blue-800">ORÇAMENTO</h2>
            ...
            <div class="flex justify-end mb-6 break-inside-avoid">
                <div class="w-1/2 bg-gray-50 p-4 rounded-lg border">
                    <div class="flex justify-between text-sm mb-1 text-gray-600">
                        <span>Subtotal:</span>
                        <span>${formatMoney(valorTotal + (descontoTotal || 0) - (acrescimoValor || 0))}</span>
                    </div>
                    ${descontoTotal > 0 ? `
                    <div class="flex justify-between text-sm mb-1 text-green-600">
                        <span>Desconto Total:</span>
                        <span>- ${formatMoney(descontoTotal)}</span>
                    </div>` : ''}
                    ${acrescimoValor > 0 ? `
        ...
        <script>
            function downloadHtml() {
                ...
                a.href = url;
                a.download = 'Orcamento_A4_${dadosPedido.venda_id}.html';
    ...
    if (win) {
        ...
        alert("Permita popups para visualizar o orçamento A4.");
    }
};
...
window.imprimirComprovanteTexto = function () {
  try {
    ...
    // Encode URL (agora o texto já está sanitizado)
    console.log('[Print] 📝 Texto sanitizado (tamanho):', texto.length);
    ...
    // --- DETECÇÃO DE FALHA (Fallback) ---
    // Se o app estiver instalado, o navegador perde o foco ou a página fica em segundo plano.
    // Se o app NÃO estiver instalado, o usuário continuará vendo esta página.
    const start = Date.now();
    setTimeout(() => {
      // Se passou menos de 2.5s e o usuário ainda está focado na página
      if (Date.now() - start < 3000) {
        console.log("[Print] ⏳ App de impressão não detectado. Oferecendo fallback...");
        
        if (confirm("O aplicativo de impressão Pulse Print não foi detectado neste dispositivo.\n\nDeseja utilizar a impressão padrão do navegador (Imagem/PDF)?")) {
           if (typeof window.imprimirComprovante === 'function') {
             window.imprimirComprovante();
           } else {
             alert("Função de impressão padrão não encontrada.");
           }
        }
      }
    }, 2000);
...
window.compartilharComprovanteImagem = async function () {
  ...
  try {
    if (navigator.share) {
      ...
      alert(
        "Seu navegador não suporta compartilhamento direto. Pressione e segure na imagem para salvar/compartilhar.",
      );
    }
  } catch (e) {
