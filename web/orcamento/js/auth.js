...
export async function verificarAutenticacao() {
    try {
        ...
        // Se não tem token e não tem cookie de sessão (assumido que se não tem token, pode falhar),
        // mas tentamos mesmo assim caso ainda esteja usando cookie (transição)
        
        const response = await fetch(`${CONFIG.URL_API}/api/usuario/me`, {
            method: 'GET',
            // credentials: 'include', // Removido para forçar uso do Token, ou manter se quiser híbrido
            headers: headers
        });
        ...
        if (response.status === 401 || response.status === 403) {
            ...
            // window.location.href = `${CONFIG.URL_API}/auth/login`; // Antigo
            // Agora disparamos evento para mostrar login modal
            window.dispatchEvent(new CustomEvent('exibir-login-modal'));
            return null;
        }
