...
async function carregarProdutos(pagina = 1, forcarRecarregar = false, termoBusca = '') {
    try {
        ...
        // ✅ UPDATE: Aumentado para 100 itens por página e expande variações (e suas fotos), fotos do pai e categoria
...
window.limparTodoOCarrinho = async function() {
    if (confirm('Tem certeza que deseja remover todos os itens do carrinho? Esta ação não pode ser desfeita.')) {
        try {
            ...
            // Garantir que os badges nos cards de produtos sumam
            document.querySelectorAll('.badge-no-carrinho').forEach(badge => {
                badge.classList.add('hidden');
            });

            console.log('[Carrinho] Limpeza total realizada com sucesso.');
        } catch (error) {
...
window.abrirModalVariacoes = async function(produtoId) {
    ...
    try {
        ...
        const data = await response.json();
        // A API success() envelopa em .data
