...
export function inicializarGaleria() {
    ...
    // Botão fechar (desktop/mobile no topo)
    const btnFechar = document.getElementById('btn-fechar-galeria');
    if (btnFechar) {
        btnFechar.addEventListener('click', fecharGaleria);
    }

    // Botão fechar mobile (rodapé)
    const btnFecharMobile = document.getElementById('btn-fechar-galeria-mobile');
    if (btnFecharMobile) {
        btnFecharMobile.addEventListener('click', fecharGaleria);
    }
