...
// ==================================================================
const getLojaId = () => {
    // self.location.pathname aqui é /pulse/basic/web/venda-direta/sw.js
    const pathname = self.location.pathname;
    const segments = pathname.split('/').filter(p => p);
    // segments = ['pulse', 'basic', 'web', 'venda-direta', 'sw.js']
    const lojaPath = segments[segments.length - 2]; // Pega 'venda-direta'
    ...
    return lojaMap[lojaPath] || lojaMap['venda-direta']; // Retorna o ID
};
...
// ✅ AJUSTE: Cache específico para venda-direta (atualizado para forçar refresh)
const CACHE_NAME = 'venda-direta-cache-v2'; 
...
async function enviarPedidosPendentes() {
    let pedidoPendente;
    try {
        // Agora idbKeyval usa o DB correto ('venda-direta-db') e o escopo correto ('self')
