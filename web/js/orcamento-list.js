        ...
        <div class="max-w-2xl mx-auto bg-white">
            ...
            <div class="flex flex-wrap gap-2 mt-6 no-print" id="botoes-acao-recibo">
                ...
                <div class="w-full flex gap-2 mt-2">
                    <button onclick="compartilharOrcamentoWhatsApp('${dados.id}', '${dados.cliente ? (dados.cliente.telefone || '') : ''}', '${dados.hash}')" class="flex-1 flex items-center justify-center bg-green-500 text-white py-2 rounded-lg font-bold hover:bg-green-600 transition-colors" title="Compartilhar Link no WhatsApp">
                    ...
                    <button onclick="compartilharOrcamentoWhatsAppImagem('${dados.cliente ? (dados.cliente.telefone || '') : ''}')" class="flex-1 flex items-center justify-center bg-purple-600 text-white py-2 rounded-lg font-bold hover:bg-purple-700 transition-colors shadow-md" title="Enviar Imagem pela Evolution API">
                        <svg class="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 24 24"><path d="M12.031 6.172c-3.181 0-5.767 2.586-5.768 5.766-.001 1.298.38 2.27 1.038 3.284l-.542 2.317 2.357-.546c1.021.575 1.914.911 3.127.912h.001c3.181 0 5.767-2.586 5.768-5.766 0-3.18-2.586-5.766-5.767-5.767zm3.391 8.221c-.142.405-.815.739-1.121.78-.306.04-.685.074-1.747-.356-1.207-.492-2.198-1.554-2.718-2.223-.06-.078-.105-.139-.136-.188-.28-.399-.497-.707-.497-1.112 0-.441.228-.68.32-.78l.069-.074c.056-.063.155-.121.274-.121.117 0 .225.011.302.012.077.001.144.02.212.182.107.258.351.854.382.918.031.064.051.139.011.22-.04.082-.06.139-.121.21-.061.071-.129.158-.184.214-.059.06-.121.124-.052.245.069.121.306.505.657.817.452.401.832.525.952.585.12.06.192.051.264-.03s.306-.356.387-.478c.081-.121.162-.102.274-.061.112.041.711.335.832.396.121.06.202.09.232.141.03.05.03.295-.112.699zM12 1c-6.075 0-11 4.925-11 11s4.925 11 11 11 11-4.925 11-11-4.925-11-11-11zm0 2c4.97 0 9 4.03 9 9s-4.03 9-9 9-9-4.03-9-9 4.03-9 9-9z"/></svg>
                        WhatsApp (Imagem)
                    </button>
                </div>
...
// =========================================================================
// WhatsApp (Imagem) - Integração Evolution API com HTML2Canvas para Orçamento
// =========================================================================
window.compartilharOrcamentoWhatsAppImagem = async function(telefonePadrao = '') {
    ...
    overlay.innerHTML = `
        ...
        <span>Fotografando orçamento e enviando via Evolution API...</span>
    `;
    document.body.appendChild(overlay);

    try {
        // 3. Injeta html2canvas se necessário (já vem no php, mas garante)
        ...
        const response = await fetch(`${baseUrl}/api/whatsapp/send`, {
            ...
            body: JSON.stringify({
                numero: numeroLimpo,
                mensagem: "Segue o seu orçamento detalhado. Qualquer dúvida, estou à disposição!",
                base64: base64data
            })
        });
        ...
        if (response.ok && resData.success) {
            alert("✅ Orçamento em imagem enviado com sucesso via WhatsApp!");
        } else {
