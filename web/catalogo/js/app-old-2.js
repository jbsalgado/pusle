...
function obterElementosDOM() {
    return {
        ...
        // === NOVO: Campos de data e intervalo ===
...
// === ATUALIZADA: Alternar visibilidade dos campos de data e intervalo ===
function alternarCampoDataPrimeiroPagamento(el) {
    ...
    if (numeroParcelas > 1) {
        // Mostra os campos de data e intervalo
...
async function confirmarPedido(el) {
    ...
    if (numeroParcelas > 1) {
        ...
        // Valida se a data não é anterior a hoje (comparação correta)
        ...
        // === NOVA VALIDAÇÃO: Intervalo entre parcelas ===
        const intervaloDias = parseInt(el.inputIntervaloParcelas?.value, 10);
        console.log('🔍 DEBUG - Intervalo de dias:', intervaloDias);
    ...
    // === ADICIONA: Data do primeiro pagamento e intervalo se houver parcelas ===
    if (numeroParcelas > 1) {
        if (el.inputDataPrimeiroPagamento?.value) {
        ...
        if (el.inputIntervaloParcelas?.value) {
            ...
            console.log('✅ DEBUG - Campo intervalo_dias_parcelas adicionado:', dadosPedido.intervalo_dias_parcelas);
        }
        
        if (!dadosPedido.data_primeiro_pagamento || !dadosPedido.intervalo_dias_parcelas) {
            console.error('❌ DEBUG - Parcelas > 1 mas faltam dados!');
        }
    }
...
// === ATUALIZADA: Reset do formulário incluindo campo de data e intervalo ===
function resetarFormularioPedido(el) {
    ...
    // === Reset dos campos de data e intervalo ===
